...
namespace CQRS.Demo.Query.Repositories
{
    public class BaseRepository
    {
        ...
        // first part of the key created by the repository e.g. location or employee
        ...
        {
            var serializedObject = GetObject(keySuffix);
        ...
        {
            return GetObject(keySuffix).IsNullOrEmpty;
        }
        ...
        private RedisValue GetObject(string keySuffix)
        {
            var key = MakeKey(keySuffix);
            var database = _redisConnection.GetDatabase();
